...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        distance = math.sqrt((self.circle.center.x - p.x)**2 + (self.circle.center.y - p.y)**2)
        return distance
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        rn = random.randrange(3, 16)
        self.circle.outline_thickness = rn
    ...
    def swell_or_shrink_repeatedly(self,
        ...
        for k in range(times_to_swell_or_shrink):
            ...
            self.circle.radius = self.circle.radius
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        center = rg.Point((self.center.x + other_circle_changer.center.x)/2, (self.center.y + other_circle_changer.center.y)/2)
        r = int(math.sqrt((self.center.x - other_circle_changer.center.x)**2 + (self.center.y - other_circle_changer.center.y)**2) / 2)
        circle = rg.Circle(center, r)
        colors = self.colors + other_circle_changer.colors
        return CircleChanger(circle.center.x, circle.center.y, r, 'red',colors)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        for k in range(self.change):
            if k < len(self.colors):
                self.circle.fill_color = self.colors[k]
            if k >= len(self.colors):
                self.circle.fill_color = self.colors[k % len(self.colors)]
